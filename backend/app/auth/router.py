...
from app.auth.mock_data import (
    MOCK_ACCESS_TOKEN,
    MOCK_REFRESH_TOKEN,
    MOCK_USER,
)
...
async def login(body: TokenRequest) -> TokenResponse:
    """Issue mock access and refresh tokens after validating credentials."""
    if body.email != MOCK_USER["email"] or body.password != MOCK_USER["password"]:
        raise HTTPException(
...
async def refresh_token(body: RefreshRequest) -> TokenResponse:
    """Issue a new mock token pair from a valid refresh token."""
